...
import {Harberger, Perwei} from "../src/Harberger.sol";

contract HarbergerTest is Test {
  function testPriceZero() public pure {
  ...
  function testUsedBuffer() public pure {
    uint256 blockDiff = 50;
  ...
  function testLowerPrice() public pure {
  ...
  function testConsumingTotalPrice() public pure {
  ...
  function testGettingNextPrice() public pure {
    uint256 blockDiff = 1;
    Perwei memory perwei1 = Perwei(1, 100);
    uint256 price = 1 ether;
  ...
  function testBlockTax() public pure {
    uint256 blockDiff1 = 1;
    Perwei memory perwei1 = Perwei(1, 100);
    assertEq(Harberger.taxPerSecond(perwei1, blockDiff1, 1 ether), 0.01 ether);

    uint256 blockDiff2 = 2;
    Perwei memory perwei2 = Perwei(1, 100);
    assertEq(Harberger.taxPerSecond(perwei2, blockDiff2, 1 ether), 0.02 ether);

    uint256 blockDiff3 = 100;
    Perwei memory perwei3 = Perwei(1, 100);
    assertEq(Harberger.taxPerSecond(perwei3, blockDiff3, 1 ether), 1 ether);

    uint256 blockDiff4 = 2;
    Perwei memory perwei4 = Perwei(100, 100);
    assertEq(Harberger.taxPerSecond(perwei4, blockDiff4, 1 ether), 2 ether);

    uint256 blockDiff5 = 1;
    Perwei memory perwei5 = Perwei(1, 1000);
    assertEq(Harberger.taxPerSecond(perwei5, blockDiff5, 1 ether), 0.001 ether);
    ...
    assertEq(Harberger.taxPerSecond(perwei6, blockDiff6, 1 ether), 1 wei);
  }
}
